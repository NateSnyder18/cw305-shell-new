...
// ==============================================================
`timescale 1ns/1ps
module AES128Encrypt_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER               = 6'h08,
    ADDR_ISR               = 6'h0c,
    ADDR_KEY_DATA_0        = 6'h10,
    ADDR_KEY_CTRL          = 6'h14,
    ADDR_PLAINTEXT_DATA_0  = 6'h18,
    ADDR_PLAINTEXT_CTRL    = 6'h1c,
    ADDR_CIPHERTEXT_DATA_0 = 6'h20,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_key = 'b0;
    reg  [31:0]                   int_plaintext = 'b0;
    reg  [31:0]                   int_ciphertext = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CIPHERTEXT_DATA_0: begin
                    rdata <= int_ciphertext[31:0];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_plaintext[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PLAINTEXT_DATA_0)
            int_plaintext[31:0] <= (WDATA[31:0] & wmask) | (int_plaintext[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_ciphertext[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CIPHERTEXT_DATA_0)
            int_ciphertext[31:0] <= (WDATA[31:0] & wmask) | (int_ciphertext[31:0] & ~wmask);
    end
end
